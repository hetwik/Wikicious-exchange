 ...
 *
 * This contract solves it by making the rules *automatic* and *on-chain*:
...
contract WikiDynamicLeverage is Ownable2Step, Pausable {
    ...
    function updateLeverageCaps() external whenNotPaused {
        ...
        if (newTierIdx > oldTierIdx) {
            emit TierAdvanced(newTierIdx, newTier.name, newTier.maxLeverage,
                newTier.maxPositionUsdc, fund);
